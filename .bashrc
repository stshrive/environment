...
settings=$(find ~ -maxdepth 2 -path '*/.bash/*.settings' -type f)
...
profiles=$(cut -f 1 ~/.bash/.profiles)
for _profile in ${profiles[@]}; do
    [ -d "$HOME/.bash/profiles/$_profile" ] && source ~/.bash/profiles/$_profile/.bashrc
    [ -f "$HOME/.bash/profiles/$_profile/env.settings" ] && export $(cut -f 1 ~/.bash/profiles/$_profile/env.settings)
done
